...
json字段通过匹配的名字和结构与pb字段一一对应。json中一定要包含pb的required字段，否则转化会失败，对应请求会被拒绝。json中可以包含pb中没有定义的字段，但它们会被丢弃而不会存入pb的unknown字段。转化规则详见[json <=> protobuf](https://brpc.apache.org/zh/docs/server/json2pb/)。
...
- [百度标准协议](https://brpc.apache.org/zh/docs/server/basics/baidu_std.md)，显示为"baidu_std"，默认启用。

- [流式RPC协议](https://brpc.apache.org/zh/docs/client/streaming-rpc/)，显示为"streaming_rpc", 默认启用。
...
- 和UB相关的协议请阅读[实现NsheadService](https://brpc.apache.org/zh/docs/server/basics/nshead_service.md)。
...
注意附件不会被压缩。HTTP body的压缩方法见[这里](https://brpc.apache.org/zh/docs/server/serve-httph2/#压缩response-body)。
...
更具体的性能对比见[Client-压缩](https://brpc.apache.org/zh/docs/client/basics/#压缩).
...
- SSL开启后，端口仍然支持非SSL的连接访问，Server会自动判断哪些是SSL，哪些不是。如果要屏蔽非SSL访问，用户可通过`Controller::is_ssl()`判断是否是SSL，同时在[connections](https://brpc.apache.org/zh/docs/builtin-services/connections/)内置监控上也可以看到连接的SSL信息。
...
最大并发度 = 极限QPS * 低负载延时 ([little’s law](https://en.wikipedia.org/wiki/Little's_law))
...
关于自适应限流的更多细节可以看[这里](https://brpc.apache.org/zh/docs/server/auto-concurrencylimiter/)
...
### Q: Fail to write into fd=1865 [SocketId=8905@10.208.245.43](mailto:SocketId=8905@10.208.245.43):54742@8230: Got EOF是什么意思
...
### Q: Remote side of fd=9 [SocketId=2@10.94.66.55](mailto:SocketId=2@10.94.66.55):8000 was closed是什么意思

这不是错误，是常见的warning，表示对端关掉连接了（EOF)。这个日志有时对排查问题有帮助。

默认关闭，把参数-log_connection_close设置为true就打开了（支持[动态修改](https://brpc.apache.org/zh/docs/builtin-services/flags/#change-gflag-on-the-fly)）。

### Q: 为什么server端线程数设了没用

brpc同一个进程中所有的server[共用线程](https://brpc.apache.org/zh/docs/server/basics/#worker线程数)，如果创建了多个server，最终的工作线程数多半是最大的那个ServerOptions.num_threads。

### Q: 为什么client端的延时远大于server端的延时

可能是server端的工作线程不够用了，出现了排队现象。排查方法请查看[高效率排查服务卡顿](https://brpc.apache.org/zh/docs/server/debug-server-issues/)。
...
![img](https://brpc.apache.org/images/docs/server_side.png)

修改于 2023年5月16日: [add security bug fix pages (#148) (a29da9f)](https://github.com/apache/brpc-website/commit/a29da9f83009053aa299a57936166bef0d01a9a2)